"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.GuestsController = void 0;
const common_1 = require("@nestjs/common");
const swagger_1 = require("@nestjs/swagger");
const guests_dto_1 = require("./guests.dto");
let GuestsController = class GuestsController {
    findAll(eventId) {
        return [];
    }
    create(eventId, dto) {
        return {};
    }
    update(eventId, guestId, dto) {
        return {};
    }
    remove(eventId, guestId) {
    }
};
exports.GuestsController = GuestsController;
__decorate([
    (0, common_1.Get)(),
    (0, swagger_1.ApiOperation)({ summary: 'Lista convidados do evento' }),
    (0, swagger_1.ApiResponse)({ status: 200, description: 'Lista de convidados' }),
    __param(0, (0, common_1.Param)('id')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String]),
    __metadata("design:returntype", void 0)
], GuestsController.prototype, "findAll", null);
__decorate([
    (0, common_1.Post)(),
    (0, swagger_1.ApiOperation)({ summary: 'Adiciona convidado ao evento' }),
    (0, swagger_1.ApiResponse)({ status: 201, description: 'Convidado adicionado' }),
    __param(0, (0, common_1.Param)('id')),
    __param(1, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String, guests_dto_1.CreateGuestDto]),
    __metadata("design:returntype", void 0)
], GuestsController.prototype, "create", null);
__decorate([
    (0, common_1.Patch)(':guestId'),
    (0, swagger_1.ApiOperation)({ summary: 'Atualiza status de presença' }),
    (0, swagger_1.ApiResponse)({ status: 200, description: 'Status atualizado' }),
    __param(0, (0, common_1.Param)('id')),
    __param(1, (0, common_1.Param)('guestId')),
    __param(2, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String, String, guests_dto_1.UpdateGuestDto]),
    __metadata("design:returntype", void 0)
], GuestsController.prototype, "update", null);
__decorate([
    (0, common_1.Delete)(':guestId'),
    (0, common_1.HttpCode)(204),
    (0, swagger_1.ApiOperation)({ summary: 'Remove convidado' }),
    (0, swagger_1.ApiResponse)({ status: 204, description: 'Removido' }),
    __param(0, (0, common_1.Param)('id')),
    __param(1, (0, common_1.Param)('guestId')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String, String]),
    __metadata("design:returntype", void 0)
], GuestsController.prototype, "remove", null);
exports.GuestsController = GuestsController = __decorate([
    (0, swagger_1.ApiTags)('Guests'),
    (0, swagger_1.ApiBearerAuth)(),
    (0, common_1.Controller)('events/:id/guests')
], GuestsController);
//# sourceMappingURL=guests.controller.js.map